...
export default function App() {
    ...
    const listener = (message: any) => {
      if (message.type === 'ELEMENT_SELECTED') {
        ...
        const newField: Field = {
          ...
          selector, relativeSelector, childIndex,
          preview: text, matchCount: matchCount || 0,
          elementType, extractMode: 'text' as const, previewUrl, previewText
        }
  ...
  const sendMessageToTab = async (tabId: number, message: any) => {
    try {
      ...
      } catch (e) { /* Ping failed — inject below */ }
  ...
  const toggleSelection = async () => {
    try {
      ...
      if ((err as Error).message === 'RESTRICTED_URL') { setIsSelecting(false); return }
  ...
  const toggleParentSelection = async () => {
    try {
      ...
      if ((err as Error).message === 'RESTRICTED_URL') { setIsSelectingParent(false); return }
      console.error('Xtractify: Connection failed', err)
      alert('Could not connect to the page. Please refresh the page being scraped and try again.')
      setIsSelectingParent(false)
    }
  ...
  const extractData = async () => {
    try {
      ...
      const response = await sendMessageToTab(tab.id, {
        type: 'EXTRACT_DATA', payload: { fields, parentSelector }
      })
      if (!response) { alert('Could not communicate with the page. Please wait for it to load completely or refresh it.'); return }
      if (response.results) {
        const newMaxLength = Math.max(0, ...response.results.map((r: any) => r.values?.length || 0))
        if (newMaxLength === 0) { alert('Zero records found on this page matching your selections.'); return }
        let addedCount = 0
        setResults((prev: ExtractionResult[]) => {
          if (prev.length === 0) { addedCount = newMaxLength; return response.results }
          ...
          for (let i = 0; i < newMaxLength; i++) {
            if (acceptDuplicates) { validIndices.push(i) }
            else {
              const hash = JSON.stringify(prev.map(p => {
                const newRes = response.results.find((r: any) => r.id === p.id)
                return newRes ? (newRes.values[i] || '') : ''
              }))
              if (!existingHashes.has(hash)) { validIndices.push(i); existingHashes.add(hash) }
            }
          }
          ...
          return prev.map(p => {
            ...
            if (newRes) {
              return { ...p, values: [...p.values, ...validIndices.map(i => newRes.values[i] ?? '')] }
            }
            return p
          })
        })
  ...
  const getDocuments = () => {
    ...
    for (let i = 0; i < maxLength; i++) {
      ...
      results.forEach(r => { doc[r.name] = r.values[i] || '' })
      docs.push(doc)
    }
    return docs
  }
  ...
  return (
    <div className="app-container">
      <main className="main">
        ...
        <section className="actions">
          <div className="action-grid">
            <button className={`btn-primary ${isSelectingParent ? 'active' : ''}`} onClick={toggleParentSelection}>
              {isSelectingParent ? 'Stop Selecting Container' : 'Set Item Container'}
            </button>
            <button className={`btn-primary ${isSelecting ? 'active' : ''}`} onClick={toggleSelection}
              disabled={!parentSelector} title={!parentSelector ? "Set an Item Container first to select fields" : ""}>
          ...
          {parentSelector && (
            <div className="parent-info-card">
              <div className="parent-meta-row">
                ...
                {parentCount !== null && (<span className="parent-count-badge">{parentCount} containers found</span>)}
              </div>
              <span className="parent-selector">{parentSelector}</span>
              <button className="btn-clear" onClick={() => { setParentSelector(null); setParentCount(null); setFields([]) }}>×</button>
            </div>
          )}
        </section>
        ...
        <section className="fields-section">
          ...
          <div className="fields-list">
              ...
              <div key={field.id} className="field-card">
                <div className="field-info">
                  <div className="field-header">
                    <input type="text" className="field-name-edit" value={field.name}
                    ...
                    <div className="field-actions">
                      ...
                      {(field.elementType === 'image' || field.elementType === 'link') && (
                        <button className="btn-icon" title="Extract Other Mode"
                          onClick={() => {
                  ...
                  <div className="field-meta">
                    <span className={`match-badge ${field.matchCount > 200 ? 'warning' : ''}`}>{field.matchCount} matched</span>
                    {(field.elementType === 'image' || field.elementType === 'link') && (
                      <div className="mode-toggle">
                        <button className={`mode-btn ${field.extractMode === 'text' ? 'active' : ''}`}
                          onClick={() => { setFields(fields.map(f => f.id === field.id ? { ...f, extractMode: 'text' as const } : f)) }}>Text</button>
                        <button className={`mode-btn ${field.extractMode === 'url' ? 'active' : ''}`}
                          onClick={() => { setFields(fields.map(f => f.id === field.id ? { ...f, extractMode: 'url' as const } : f)) }}>URL</button>
                      </div>
                    )}
                  </div>
            ...
            {fields.length === 0 && (<div className="empty-state">Select item container first, then fields.</div>)}
          </div>
        </section>
          ...
          <section className="extraction-section">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.8rem', color: 'var(--text-secondary)', cursor: 'pointer' }}>
                <input type="checkbox" checked={acceptDuplicates} onChange={(e) => setAcceptDuplicates(e.target.checked)}
                  style={{ accentColor: 'var(--accent-color)', width: '14px', height: '14px', cursor: 'pointer' }} />
                Accept duplicate data
              </label>
              <button className="btn-secondary" onClick={extractData}>Extract Data</button>
            </div>
              ...
              <div className="results-container fade-in">
                <div className="results-header-summary">
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button className="btn-download-small" onClick={() => { if (confirm('Are you sure you want to clear all extracted data?')) setResults([]) }}
                      style={{ background: 'var(--input-bg)', color: 'var(--text-secondary)', border: '1px solid var(--border-color)' }}>Clear</button>
                    <button className="btn-download-small" onClick={downloadCSV}>Download CSV</button>
                    <button className="btn-download-small" onClick={downloadJSON} style={{ background: 'var(--text-primary)', color: 'var(--bg-color)' }}>Download JSON</button>
                  </div>
                </div>
                <div className="examples-accordion">
                  <button className="btn-toggle-examples" onClick={() => setShowExamples(!showExamples)}>
                    {showExamples ? 'Hide Examples ↑' : 'See Examples ↓'}
                  </button>
                  {showExamples && (
                    <div className="table-container fade-in">
                      <table className="results-table">
                        <thead><tr><th>#</th>{results.map(r => <th key={r.id}>{r.name}</th>)}</tr></thead>
                        <tbody>
                            ...
                            <tr key={idx}>
                              ...
                              {results.map(r => (
                                <td key={r.id}
                                  ...
                                  onMouseLeave={() => setHoveredImage(null)}>{doc[r.name]}</td>
                              ))}
                            </tr>
                          ))}
                        </tbody>
                      ...
                      <div className="table-footer">
                        ...
                        {visibleCount < totalRecords && (
                          <button className="btn-load-more" onClick={() => setVisibleCount(prev => prev + 10)}>Load More</button>
                        )}
                      </div>
                    </div>
                  )}
      ...
      {hoveredImage && (
        <div style={{ position: 'fixed', left: hoveredImage.x, top: hoveredImage.y - 10, transform: 'translate(-50%, -100%)',
          backgroundColor: 'var(--card-bg)', padding: '4px', border: '1px solid var(--border-color)', borderRadius: '4px',
          zIndex: 9999, boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)', pointerEvents: 'none', maxWidth: '200px' }}>
          <img src={hoveredImage.url} alt="Preview"
            style={{ maxWidth: '100%', height: 'auto', display: 'block', borderRadius: '2px', objectFit: 'contain' }}
            onError={(e) => { const parent = e.currentTarget.parentElement; if (parent) parent.style.display = 'none' }} />
        </div>
      )}
    </div>
  )
